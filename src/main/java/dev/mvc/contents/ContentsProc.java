...
//      str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
        ...
        if (now_grp >= 2) { // 현재 그룹번호가 2이상이면 페이지수가 11페이 이상임으로 이전 그룹으로 갈수 있는 링크 생성
            str.append("<span class='span_box_1'><A href='" + Contents.LIST_FILE + "?&word=" + word + "&now_page="
                    + _now_page + "&cateno=" + cateno + "'>이전</A></span>");
        }
        ...
        if (now_grp < total_grp) {
            str.append("<span class='span_box_1'><A href='" + Contents.LIST_FILE + "?&word=" + word + "&now_page="
                    + _now_page + "&cateno=" + cateno + "'>다음</A></span>");
        }
        str.append("</DIV>");
